...
describe('UserDetailsTemplate', () => {
  ...
  test('renders loading state initially', () => {
    ...
    render(
      <MemoryRouter initialEntries={['/users/12345']}>
        <UserDetailsTemplate />
      </MemoryRouter>
    )
  ...
  test('calls fetchUser with the correct id and renders user details', async () => {
    ...
    render(
      <MemoryRouter initialEntries={[`/users/4123`]}>
        <UserDetailsTemplate />
      </MemoryRouter>
    )
    ...
    expect(screen.getByText(/Gemma Goddard's Information/i)).toBeInTheDocument()
    expect(screen.getByText(/UIN: 4123/i)).toBeInTheDocument()
    expect(
      screen.getByText(/Major: Aerospace Engineering/i)
    ).toBeInTheDocument()
    expect(screen.getByText(/Email: gemgoddard@yahoo.com/i)).toBeInTheDocument()
  })
